...
<a href="{{ route('kendaraan.daftar_kendaraan', ['page' => $currentPage, 'search' => request()->query('search')]) }}"
    ...
    <div class="min-h-screen flex items-start justify-center sm:pt-3 px-4 sm:px-8 pb-8">
        <div class="max-w-4xl w-full">
        <div class="max-w-4xl w-full bg-white p-4 sm:p-8 md:p-12 rounded-lg shadow-lg">
            <h2 class="text-xl sm:text-2xl font-bold mb-4 sm:mb-6 text-center">Form Tambah Kendaraan</h2>
            <form id="save-form" action="{{ route('kendaraan.store') }}" method="POST" enctype="multipart/form-data" novalidate>
...
function patchValidationFunctions() {
    ...
    vitalFields.forEach(field => {
        ...
        if (input) {
            ...
            newInput.dataset.isTouched = 'false';
            
            const warningElement = document.getElementById(field.warningId);
            
            newInput.addEventListener('input', function() {
                this.dataset.isTouched = 'true';
                
                if (!this.value.trim() && this.dataset.isTouched === 'true') {
                    showWarning(this, warningElement, `${field.label} harus diisi!`);
            ...
            if (!newInput.value.trim() && newInput.dataset.isTouched === 'true') {
...
function setupRealTimeValidation() {
    ...
    Object.keys(fieldLimits).forEach(fieldName => {
        ...
        if (input) {
            ...
            newInput.addEventListener('input', function() {
                ...
                } else if (config.required && !this.value.trim() && this.dataset.isTouched === 'true') {
            ...
            if (config.required && !newInput.value.trim() && newInput.dataset.isTouched === 'true') {
                const warningElement = document.getElementById(config.warningId);
                const message = `${fieldName} harus diisi!`;
    ...
    if (platNomorInput) {
        ...
        newPlatNomorInput.addEventListener('input', function() {
            ...
            } else if (!platNomor && this.dataset.isTouched === 'true') {
        ...
        newPlatNomorInput.addEventListener('blur', function() {
            const warningElement = document.getElementById('warning-plat-nomor');
            this.dataset.isTouched = 'true'; 
    ...
    integerFields.forEach(field => {
        ...
        if (input) {
            ...
            newInput.addEventListener('input', function() {
                ...
                if (value === 0 && this.dataset.isTouched === 'true') {
                    const message = `${field.name} harus diisi dan minimal 1!`;
                    showWarning(this, warningElement, message);
                } else if (value > field.maxValue) {
                    const message = `${field.name} maksimal ${field.maxValue} karakter!`;
                    showWarning(this, warningElement, message);
                } else {
                    hideWarning(this, warningElement);
                }
            ...
            if (value === 0 && newInput.dataset.isTouched === 'true') {
                const warningElement = document.getElementById(field.warningId);
                const message = `${field.name} harus diisi dan minimal 1!`;
                showWarning(newInput, warningElement, message);
            }
    ...
    currencyFields.forEach(field => {
        ...
        if (input) {
            ...
            const validate = (value) => {
                ...
                if (!numericValue || BigInt(numericValue) === 0n) {
                    return `${field.name} harus diisi dan tidak boleh 0!`;
                }
                try {
                    const bigIntValue = BigInt(numericValue);
                    if (bigIntValue > MAX_VALUE) {
                        return `${field.name} tidak boleh lebih dari 9.223.372.036.854.775.807!`;
                    }
                } catch {
                    return `Format ${field.name} tidak valid!`;
                }
                return null;
            };
            ...
            newInput.addEventListener('input', function () {
                ...
                this.dataset.numericValue = this.value.replace(/[^\d]/g, '');
            });
