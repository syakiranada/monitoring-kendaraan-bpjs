<x-app-layout>
    ...
    document.getElementById('save-form').addEventListener('submit', function(event) {
        ...
        let currentVehicleId = {{ $kendaraan->id_kendaraan }};
        ...
        let dateFields = [
            ...
            { name: 'tanggal_asuransi', label: 'Tanggal Asuransi Terakhir' },
        ...
        if (tanggalAsuransi || tanggalPerlindunganAwal || tanggalPerlindunganAkhir) {
            if (!tanggalAsuransi) {
                alertDiv.innerHTML = '<span class="font-medium">Peringatan!</span> Tanggal Bayar Asuransi harus diisi.';
                alertDiv.classList.remove('hidden');
                alertDiv.scrollIntoView({ behavior: 'smooth', block: 'center' });
                setTimeout(() => alertDiv.classList.add('hidden'), 10000);
                return;
            }
            if (!tanggalPerlindunganAwal) {
                alertDiv.innerHTML = '<span class="font-medium">Peringatan!</span> Tanggal Perlindungan Awal harus diisi.';
                alertDiv.classList.remove('hidden');
                alertDiv.scrollIntoView({ behavior: 'smooth', block: 'center' });
                setTimeout(() => alertDiv.classList.add('hidden'), 10000);
                return;
            }
            if (!tanggalPerlindunganAkhir) {
                alertDiv.innerHTML = '<span class="font-medium">Peringatan!</span> Tanggal Perlindungan Akhir harus diisi.';
                alertDiv.classList.remove('hidden');
                alertDiv.scrollIntoView({ behavior: 'smooth', block: 'center' });
                setTimeout(() => alertDiv.classList.add('hidden'), 10000);
                return;
            ...
            if (new Date(tanggalPerlindunganAkhir) <= new Date(tanggalPerlindunganAwal)) {
            ...
            if (new Date(tanggalAsuransi) > new Date(today)) {
                alertDiv.innerHTML = '<span class="font-medium">Peringatan!</span> Tanggal Bayar Asuransi tidak boleh lebih dari hari ini.';
                alertDiv.classList.remove('hidden');
                alertDiv.scrollIntoView({ behavior: 'smooth', block: 'center' });
                setTimeout(() => alertDiv.classList.add('hidden'), 10000);
                return;
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        Swal.fire({
                            ...
                            document.getElementById('save-form').removeEventListener('submit', arguments.callee);
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const insuranceDateFields = [
            { name: "tanggal_asuransi", label: "Tanggal Bayar Asuransi", needsMax: true },
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const rupiahInputs = document.querySelectorAll('input[oninput="formatRupiah(this)"]');
        rupiahInputs.forEach(input => {
            input.dataset.numericValue = input.value.replace(/[^\d]/g, '');
        });
    });
    </script>
</x-app-layout>