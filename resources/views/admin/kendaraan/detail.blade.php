<x-app-layout>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 px-2 md:px-0">
        <div class="space-y-3 bg-white border border-gray-200 p-3 md:p-4 rounded-lg shadow-sm">
            <h2 class="text-lg md:text-xl font-semibold text-gray-900">Detail Kendaraan</h2>
            <div class="space-y-2 md:space-y-3">
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Merk</span>
                    <span class="text-sm text-gray-900 flex-1">{{ $kendaraan->merk ?? '-' }}</span>
                </div>
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Tipe</span>
                    <span class="text-sm text-gray-900 flex-1">{{ $kendaraan->tipe ?? '-' }}</span>
                </div>
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Plat Nomor</span>
                    <span class="text-sm text-gray-900 flex-1">{{ $kendaraan->plat_nomor ?? '-' }}</span>
                </div>
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Warna</span>
                    <span class="text-sm text-gray-900 flex-1">{{ $kendaraan->warna ?? '-' }}</span>
                </div>
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Jenis Kendaraan</span>
                    <span class="text-sm text-gray-900 flex-1">{{ $kendaraan->jenis ?? '-' }}</span>
                </div>
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Status Aset</span>
                    <span class="text-sm text-gray-900 flex-1">{{ $kendaraan->aset ?? '-' }}</span>
                </div>
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Kapasitas Penumpang</span>
                    <span class="text-sm text-gray-900 flex-1">{{ $kendaraan->kapasitas ?? '-' }}</span>
                </div>
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Tanggal Pembelian</span>
                ...
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Nilai Perolehan</span>
                    <span class="text-sm text-gray-900 flex-1">{{ $kendaraan->nilai_perolehan ? 'Rp ' . number_format($kendaraan->nilai_perolehan, 0, ',', '.') : '-' }}</span>
                </div>
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Nilai Buku</span>
                    <span class="text-sm text-gray-900 flex-1">{{ $kendaraan->nilai_buku ? 'Rp ' . number_format($kendaraan->nilai_buku, 0, ',', '.') : '-' }}</span>
                </div>
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Bahan Bakar</span>
                    <span class="text-sm text-gray-900 flex-1">{{ $kendaraan->bahan_bakar ?? '-' }}</span>
                </div>
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Nomor Mesin</span>
                    <span class="text-sm text-gray-900 flex-1">{{ $kendaraan->no_mesin ?? '-' }}</span>
                </div>
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Nomor Rangka</span>
                    <span class="text-sm text-gray-900 flex-1">{{ $kendaraan->no_rangka ?? '-' }}</span>
                </div>
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Frekuensi Servis</span>
                    <span class="text-sm text-gray-900 flex-1">{{ $kendaraan->frekuensi_servis ? $kendaraan->frekuensi_servis . ' Bulan' : '-' }}</span>
                </div>
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Tanggal Servis Rutin Terakhir</span>
                ...
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Tanggal Bayar Asuransi Terakhir</span>
                    <span class="text-sm text-gray-900 flex-1">
                        {{ isset($asuransi) && $asuransi->tgl_bayar ? \Carbon\Carbon::parse($asuransi->tgl_bayar)->format('d-m-Y') : '-' }}
                    </span>
                </div>
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Masa Perlindungan Awal</span>
                    <span class="text-sm text-gray-900 flex-1">
                        {{ isset($asuransi) && $asuransi->tgl_perlindungan_awal ? \Carbon\Carbon::parse($asuransi->tgl_perlindungan_awal)->format('d-m-Y') : '-' }}
                    </span>
                </div>
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Masa Perlindungan Akhir</span>
                    <span class="text-sm text-gray-900 flex-1">
                        {{ isset($asuransi) && $asuransi->tgl_perlindungan_akhir ? \Carbon\Carbon::parse($asuransi->tgl_perlindungan_akhir)->format('d-m-Y') : '-' }}
                    </span>
                </div>
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Tanggal Bayar Pajak Terakhir</span>
                    <span class="text-sm text-gray-900 flex-1">
                        {{ isset($pajak) && $pajak->tgl_bayar ? \Carbon\Carbon::parse($pajak->tgl_bayar)->format('d-m-Y') : '-' }}
                    </span>
                </div>
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Tanggal Jatuh Tempo Pajak Terakhir</span>
                    <span class="text-sm text-gray-900 flex-1">
                        {{ isset($pajak) && $pajak->tgl_jatuh_tempo ? \Carbon\Carbon::parse($pajak->tgl_jatuh_tempo)->format('d-m-Y') : '-' }}
                    </span>
                </div>
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Tanggal Pengisian BBM Terakhir</span>
                    <span class="text-sm text-gray-900 flex-1">
                        {{ isset($bbm) && $bbm->tgl_isi ? \Carbon\Carbon::parse($bbm->tgl_isi)->format('d-m-Y') : '-' }}
                    </span>
                </div>
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Tanggal Cek Fisik Terakhir</span>
                    <span class="text-sm text-gray-900 flex-1">
                        {{ isset($cekFisik) && $cekFisik->tgl_cek_fisik ? \Carbon\Carbon::parse($cekFisik->tgl_cek_fisik)->format('d-m-Y') : '-' }}
                    </span>
                </div>
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Ketersediaan</span>
        ...
        <div class="bg-white border border-gray-200 p-3 md:p-4 rounded-lg shadow-sm">
            <h3 class="text-lg font-semibold mb-3 md:mb-4">Detail Kondisi Fisik</h3>
            <div class="space-y-2 md:space-y-3">
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Mesin</span>
                    <span class="text-sm text-gray-900 flex-1">{{ isset($cekFisik) && isset($cekFisik->mesin) ? $cekFisik->mesin : '-' }}</span>
                </div>
                
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Accu</span>
                    <span class="text-sm text-gray-900 flex-1">{{ isset($cekFisik) && isset($cekFisik->accu) ? $cekFisik->accu : '-' }}</span>
                </div>
                
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Air Radiator</span>
                    <span class="text-sm text-gray-900 flex-1">{{ isset($cekFisik) && isset($cekFisik->air_radiator) ? $cekFisik->air_radiator : '-' }}</span>
                </div>
                
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Air Wiper</span>
                    <span class="text-sm text-gray-900 flex-1">{{ isset($cekFisik) && isset($cekFisik->air_wiper) ? $cekFisik->air_wiper : '-' }}</span>
                </div>
                
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Body</span>
                    <span class="text-sm text-gray-900 flex-1">{{ isset($cekFisik) && isset($cekFisik->body) ? $cekFisik->body : '-' }}</span>
                </div>
                
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Ban</span>
                    <span class="text-sm text-gray-900 flex-1">{{ isset($cekFisik) && isset($cekFisik->ban) ? $cekFisik->ban : '-' }}</span>
                </div>
                
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Pengharum</span>
                    <span class="text-sm text-gray-900 flex-1">{{ isset($cekFisik) && isset($cekFisik->pengharum) ? $cekFisik->pengharum : '-' }}</span>
                </div>
                    
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Kondisi Keseluruhan</span>
                ...
                <div class="flex flex-col sm:flex-row sm:items-start">
                    <span class="text-sm text-gray-600 w-full sm:w-48 font-medium">Catatan</span>
                    <span class="text-sm text-gray-900 flex-1">{{ isset($cekFisik) && isset($cekFisik->catatan) ? $cekFisik->catatan : '-' }}</span>
                </div>
            </div>
            </div>
            <div class="col-span-1 md:col-span-2 mt-2 md:mt-4">
                <button type="button" 
                    onclick="window.location.href='{{ route('kendaraan.daftar_kendaraan', ['page' => $currentPage, 'search' => request()->query('search')]) }}'" 
                    class="bg-purple-600 text-white px-3 py-2 rounded-lg font-semibold hover:bg-purple-700 transition w-fit text-left text-sm md:text-base">
                    Kembali
                </button>
            </div>         
        </div>
