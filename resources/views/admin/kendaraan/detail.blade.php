    ...
    <div class="space-y-3 bg-white border border-gray-200 p-4 rounded-lg shadow-sm">
        ...
        <div class="space-y-3">
            ...
            <div class="flex items-start">
                ...
                <span class="text-sm text-gray-900 flex-1">{{ $servisRutin->tgl_servis_real ? \Carbon\Carbon::parse($servisRutin->tgl_servis_real)->format('d/m/Y') : '-' }}</span>
            </div>
            <div class="flex items-start">
                <span class="text-sm text-gray-600 w-48">Tanggal Bayar Asuransi Terakhir</span>
                <span class="text-sm text-gray-900 flex-1">{{ $asuransi->tgl_bayar ? \Carbon\Carbon::parse($asuransi->tgl_bayar)->format('d/m/Y') : '-' }}</span>
            </div>
            <div class="flex items-start">
                <span class="text-sm text-gray-600 w-48">Masa Perlindungan Awal</span>
                <span class="text-sm text-gray-900 flex-1">{{ $asuransi->tgl_perlindungan_awal ? \Carbon\Carbon::parse($asuransi->tgl_perlindungan_awal)->format('d/m/Y') : '-' }}</span>
            </div>
            <div class="flex items-start">
                <span class="text-sm text-gray-600 w-48">Masa Perlindungan Akhir</span>
                <span class="text-sm text-gray-900 flex-1">{{ $asuransi->tgl_perlindungan_akhir ? \Carbon\Carbon::parse($asuransi->tgl_perlindungan_akhir)->format('d/m/Y') : '-' }}</span>
            </div>
            <div class="flex items-start">
                <span class="text-sm text-gray-600 w-48">Tanggal Bayar Pajak Terakhir</span>
                <span class="text-sm text-gray-900 flex-1">{{ $pajak->tgl_bayar ? \Carbon\Carbon::parse($pajak->tgl_bayar)->format('d/m/Y') : '-' }}</span>
            </div>
            <div class="flex items-start">
                <span class="text-sm text-gray-600 w-48">Tanggal Jatuh Tempo Pajak Terakhir</span>
                <span class="text-sm text-gray-900 flex-1">{{ $pajak->tgl_jatuh_tempo ? \Carbon\Carbon::parse($pajak->tgl_jatuh_tempo)->format('d/m/Y') : '-' }}</span>
            </div>
            <div class="flex items-start">
                <span class="text-sm text-gray-600 w-48">Tanggal Pengisian BBM Terakhir</span>
                <span class="text-sm text-gray-900 flex-1">{{ $bbm->tgl_isi ? \Carbon\Carbon::parse($bbm->tgl_isi)->format('d/m/Y') : '-' }}</span>
            </div>
            <div class="flex items-start">
                <span class="text-sm text-gray-600 w-48">Tanggal Cek Fisik Terakhir</span>
                <span class="text-sm text-gray-900 flex-1">{{ $cekFisik->tgl_cek_fisik ? \Carbon\Carbon::parse($cekFisik->tgl_cek_fisik)->format('d/m/Y') : '-' }}</span>
            </div>
            <div class="flex items-start">
                <span class="text-sm text-gray-600 w-48">Ketersediaan</span>
                <span class="text-sm text-green-600 flex-1">{{ $kendaraan->status_ketersediaan ?? '-' }}</span>
            </div>
                    </div>
                </div>
